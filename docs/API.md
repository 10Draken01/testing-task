...
```bash
curl -X POST https://base.testing-task.online/users \
...
curl -X POST https://base.testing-task.online/tasks/<idTask>/assign \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: 3f1a9c2e-3333-4a2b-8c3d-000000000003" \
  -d '{"userIds":["<idUser>"]}'

curl -X POST https://base.testing-task.online/tasks/<idTask>/complete \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: 3f1a9c2e-4444-4a2b-8c3d-000000000004" \
  -d '{"userId":"<idUser>"}'

curl https://base.testing-task.online/tasks/<idTask>/notifications
```